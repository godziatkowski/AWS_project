...
exports.action = function( request, callback ){
    ...
    sendFileRotateRequests( {
        files: files,
        angle: parseInt( request.body.angle )
    } ).then( function(){
        callback( null, {
...
function sendFileRotateRequests( params ){
    var promises = [];
...
function sendMessageBatch( params ){
    ...
    for( var i = 0; i < params.files.length; i++ ){
        entries.push( {
            Id: i + '',
            MessageBody: JSON.stringify( { file: params.files[i],
                angle: params.angle, method: 'rotate' } )
        } );
    }
